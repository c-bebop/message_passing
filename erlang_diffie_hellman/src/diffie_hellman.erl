...
%%%       within two communication partners when exchanging key by the 
%%%       Diffie-Hellman key exchange algorithm.
...
%%%       which is the public component key for the Diffie-Hellman key
%%%       exchange algorithm. 
...
%%% @doc  Returns the value ComponentKey to the power of MySecretKey 
...
%%% @doc  Starts the Diffie-Hellman key exchange algorithm by taking P 
%%%       (a prime number), G (1 ... P - 1), MySecretKey is the secret
%%%       integer of the one who executes this function and the 
%%%       PartnerPID which is the PID of the communication partner 
%%%       with you a key exchange shall be initiated. This function 
%%%       sends the term {startKeyExchange, PublicData} to the 
%%%       PartnerPID where PublicData is of type publicData.
%%%       Afterwards, the function starts a receive construct which is
%%%       receiving the following:
%%%       {componentKey, PublicData}: 
%%%           The message including all information needed for 
%%%           computing the private shared key and then prints it out.
%%%       UnexpectedMessage:          
%%%           Prints out any unexpected incoming message and calls a 
%%%           recursion.
...
%%% @doc  Listens on Messages to start the Diffie-Hellman key exchange 
%%%       by starting the following receive construct:
%%%       {startKeyExchange, PublicData}: 
%%%           The message including all information needed to start 
%%%           the key exchange by computing the own public data which 
%%%           will then be send to the PartnerPID as follows: 
%%%           {componentKey, MyPublicData}.
%%%           Afterwards, the private shared key will be printed out 
%%%           and the function calls a recursion.
%%%       terminante:        
%%%           Prints out that this function terminates with the 
%%%           executing PID and returns ok.
%%%       UnexpectedMessage:    
%%%           Prints out any unexpected incomping message and calls a 
%%%           recursion.
%%% @end
...
%%% @doc  Starts a key exchange example by spawning the Alice process,
%%%       which executes the listenKeyExchange function with 
%%%       MySecretKey = 15, and the Bob process, which exectues the 
%%%       startKeyExchange function with P = 23, G = 5, 
%%%       MySecretKey = 6 and PartnerPID = Alice. Returns Alice and 
%%%       Bob.
%%%
startExample() ->
...
%%% @doc  Starts a key exchange remote example by spawning the Alice 
%%%       process, which executes the listenKeyExchange function with
%%%       MySecretKey = 15, and the Bob process, located on the 
%%%       RemoteNode, which exectues the startKeyExchange function 
%%%       with P = 23, G = 5, MySecretKey = 6 and PartnerPID = Alice.
